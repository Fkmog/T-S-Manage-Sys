...
export default {
  ...
  methods: {
    ...
    getBCMId() {
      ...
      let message = "是否确认将所选教学班添加到支撑课程中 ？";
    ...
    addBaseCourseInProgram() {
      ...
      for (
        let i = this.programInfoCourseCount;
        i < this.supportingClass.length;
        i++
      ) {
        ...
        .then((res) => {
          ...
          if (res.code === "SUCCESS") {
            ...
            res.rows.forEach((course) => {
              course.index = count;
              course.allTeacherName = course.teacherName;
              if (course.additionalTeacherList !== null) {
                course.additionalTeacherList.forEach((teacher) => {
                  course.allTeacherName =
                    course.allTeacherName + "," + teacher.teacherName;
                });
              }
              course.teachers = [];
              let temp = course.teacherName + "(" + course.teacherNumber + ")";
