  ...
  <div v-show="closeShow" class="submenu">
    <el-row>
      <el-col :span="2" class="columnstyle">
        <el-button @click="this.toggleSelection()" class="clearSelected" link
          >取消选择</el-button
        >
      </el-col>
      <el-col :span="16" class="columnstyle">
        <div class="numSelectedTeacher">
          已选中 {{ numSelected }} 节基础课程
        </div>
      </el-col>
      <el-col :span="4" class="columnstyle">
        <el-button @click="this.setDetail()" class="submenudeleteButton" link
...
export default {
  ...
  methods: {
      ...
      if (query !== "") {
        ...
        setTimeout(() => {
          ...
          this.detailForm = this.remoteDetail.filter((item) => {
            return item.respondentName.indexOf(query) > -1;
          });
        }, 200);
      } else {
        this.detailForm = [];
    ...
    addBaseCourseDetail(row) {
      let that = this;
      ElMessageBox.confirm("尚未添加版本信息是否确认添加？", "", {
