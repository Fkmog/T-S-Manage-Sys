  ...
  methods: {
    dragTab() {
      ...
      this.sortable = Sortable.create(tab, {
        ...
        onEnd({ newIndex, oldIndex }) {
          let a = JSON.parse(JSON.stringify(that.tempEditabel));
          // console.log("!",newIndex);
          try {
            let currTab = a.splice(oldIndex, 1)[0]; //鼠标拖拽当前的el-tabs-pane
    ...
    handleTabsEdit(targetName, action, activityName) {
      ...
      if (action === "remove") {
        ...
        if (targetName != "1") {
          ElMessageBox.confirm("是否删除当前成绩项", "注意", {
